    ...
    return (
      <div className="rounded-xl border border-slate-700 bg-slate-900 p-12 text-center">
        ...
        <p className="text-xs text-slate-600 mt-1">
          Alerts will appear here when policy rules are triggered
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {alerts.map((alert) => {
        ...
        return (
          <div
            key={alert.id}
            className={`rounded-xl border ${colors.border} ${colors.bg} p-4 transition-colors`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-3 text-[11px] text-slate-500">
                    ...
                    {alert.actionTaken && (
                      <span className="text-sky-400">Action: {alert.actionTaken}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="shrink-0 rounded-md border border-slate-600/50 bg-slate-800/50 px-3 py-1.5 text-xs font-medium text-slate-400 transition-colors hover:bg-slate-700 hover:text-slate-200"
              >
                Dismiss
              </button>
            </div>

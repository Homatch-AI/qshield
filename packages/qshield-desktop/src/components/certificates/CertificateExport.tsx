...
export function CertificateExport({ onClose, onGenerated }: CertificateExportProps) {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      await window.qshield.certificates.generate({
        sessionId,
        includeAllEvidence,
      });
      onGenerated();
          ...
          <div className="mt-2 space-y-2">
            <label className="flex items-center gap-3 rounded-lg border border-slate-700 bg-slate-800/30 px-4 py-3 cursor-pointer transition-colors hover:bg-slate-800/60">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Include all evidence records from the current session
                </p>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 rounded-lg border border-slate-700 bg-slate-800/30 px-4 py-3 cursor-pointer transition-colors hover:bg-slate-800/60">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Only include cryptographically verified evidence records
                </p>
              </div>
            </label>
          </div>
